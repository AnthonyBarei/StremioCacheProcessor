...
const PlexStoragePathEdit: React.FC = () => {
    ...
    const getPlexStoragePath = async () => {
        // Perform API GET request here
        axios.get('http://localhost:3000/api/configuration/storage-path/'+id).then((response) => {
            setPlexStoragePath(response.data.storagePath.plexPath);
            setLocalStoragePath(response.data.storagePath.path);
        }).catch((error) => {
            console.error(error);
        });
    };
    ...
    const handleSubmit = async (event: React.FormEvent) => {
        ...
        axios.put('http://localhost:3000/api/configuration/storage-path', putData).then((response) => {
                ...
                <Grid container spacing={2} sx={{ width: {sm: '100%', lg: '50%'} }}>
                    ...
                    <Grid item xs={12}>
                        <TextField
                            ...
                            onChange={(event) => setLocalStoragePath(event.target.value)}
                            autoFocus
                        />
                    </Grid>
                    ...
                    <Grid item xs={12}>
                        <FormControl required fullWidth
                            ...
                            <Select name="plexPath" value={plexStoragePath} onChange={(event) => setPlexStoragePath(event.target.value)}>
...
export default PlexStoragePathEdit;